...
if (isset($_SESSION['user_name']) && !empty($_SESSION['user_name'])) {
    ...
    // Fetch all users invited by the current user who bought the Star Package (including 'Star' and 'Star Package')
    $invitedUsersSql = "SELECT u.username, u.phone_number, u.registration_date 
                        FROM Users u
                        INNER JOIN Packages p ON u.username = p.username
                        WHERE u.invited_by = ? AND p.name LIKE 'Star%'";
        ...
        while ($invitedUserRow = $invitedUsersResult->fetch_assoc()) {
            ...
            if (!in_array($username, $addedUsernames)) {
                ...
                $dat[] = [
                    ...
                    'active' => 'Active' // Since they bought the Star Package
                ];
