...
$sql = "SELECT request_id, points_to_withdraw, request_date, username, status FROM WithdrawalRequests WHERE username LIKE '%$search%' AND whatsapp = 'whatsapp' ORDER BY request_date DESC LIMIT $start, $entriesPerPage";
...
// Calculate and return the total withdrawables only for those with 'whatsapp'
$totalSql = "SELECT SUM(points_to_withdraw) as total FROM WithdrawalRequests WHERE whatsapp = 'whatsapp'";
